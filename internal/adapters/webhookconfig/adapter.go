...
// shouldSkipWebhook returns true if this is a nightjar-owned webhook.
func shouldSkipWebhook(webhookMap map[string]interface{}, webhookName string) bool {
	// Skip if name contains "nightjar"
	if strings.Contains(strings.ToLower(webhookName), "nightjar") {
		return true
	}

	// Skip if clientConfig.service.name contains "nightjar"
	...
	if clientConfig != nil {
		...
		if service != nil {
			...
			if strings.Contains(strings.ToLower(serviceName), "nightjar") {
				return true
			}
		}
	}
