...
1. Go to the [Security Advisories](https://github.com/nightjarctl/nightjar/security/advisories) page
...
Nightjar is a **read-only policy observer**, not a policy engine. Key security properties:

### What Nightjar Can Access

- **Cluster-wide read access**: Nightjar requires `get`, `list`, `watch` on all resources to discover constraints. This is intentional and documented in RBAC manifests.
...
- Deploy Nightjar in a dedicated namespace (e.g., `nightjar-system`)
- Use the provided RBAC manifests without modification
- Do not grant Nightjar service accounts to developers
...
- Nightjar does not require any secrets for core functionality
...
3. **CRD schemas**: Nightjar parses unstructured objects; malformed CRDs may cause parsing errors (not security issues).
