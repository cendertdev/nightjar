...
Nightjar is delivered in 7 phases. Each phase is independently useful — Phase 1 alone delivers real value. The commercial layer (multi-cluster dashboard, advanced routing, analytics) is built on top of the open source core starting at Phase 7.
...
- Controller namespace: `nightjar-system`
- Helm chart name: `nightjar`
- Go module path: `github.com/nightjarctl/nightjar`
- License: Apache 2.0
...
### Deliverables
- [ ] `ConstraintProfile` CRD controller: watch for ConstraintProfile resources, configure adapters dynamically
- [ ] Generic adapter configuration: field path expressions for extracting selectors, namespaces, effects from arbitrary CRDs
- [ ] `nightjar.io/is-policy: "true"` annotation support on CRDs
- [ ] Auto-detection tuning: configurable heuristic thresholds
- [ ] Documentation: how to write a custom adapter (Go plugin or ConstraintProfile)
- [ ] Example adapters: cert-manager, Crossplane, Argo Rollouts
...
## Phase 7: Commercial Layer (Ongoing)

**Goal**: Multi-cluster management, advanced analytics, enterprise features.

### Deliverables (commercial product, not open source)
- [ ] Multi-cluster aggregation service
- [ ] Unified dashboard across clusters
- [ ] Historical constraint tracking and trend analysis
- [ ] Policy impact simulation ("which workloads will this new policy affect?")
- [ ] SSO integration for notification routing to actual humans
- [ ] Jira/ServiceNow ticket creation for unresolved constraints
- [ ] Compliance reporting and audit export
- [ ] SLA-based escalation (unresolved constraint → escalate after N hours)
