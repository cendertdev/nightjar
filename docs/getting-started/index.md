...
This section covers everything you need to get Nightjar running in your cluster.
{: .fs-6 .fw-300 }
...
Nightjar consists of three components:
...
| **CLI (nightjar)** | Query constraints, explain errors, pre-check manifests | Optional |
...
Nightjar needs cluster-wide read access to discover policies:
...
This is intentional—Nightjar must see all policy types to correlate them. The controller never modifies cluster resources (except its own CRDs).

### Optional: Policy Engines

Nightjar auto-detects installed policy engines:
...
Nightjar serves different audiences with different information:
...
1. [Installation](installation/) - Install Nightjar with Helm
