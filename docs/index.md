---
layout: default
title: Home
nav_order: 1
description: "Nightjar discovers all constraints affecting your Kubernetes workloads and explains them to developers."
permalink: /
---

<div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
  <img src="{{ '/assets/images/nightjar.png' | relative_url }}" alt="Nightjar" style="height: 100px;">
  <h1 class="fs-9" style="margin: 0;">Nightjar</h1>
</div>

Automatic constraint discovery and explanation for Kubernetes.
{: .fs-6 .fw-300 }

[Get Started](/nightjar/getting-started/){: .btn .btn-primary .fs-5 .mb-4 .mb-md-0 .mr-2 }
[View on GitHub](https://github.com/nightjarctl/nightjar){: .btn .fs-5 .mb-4 .mb-md-0 }

---

## What is Nightjar?

> *Catching bugs in the dark.*

Nightjar is a Kubernetes operator that **discovers all constraints** affecting your workloads—network policies, admission policies, resource quotas, service mesh rules, and more—and **explains them to developers** when things go wrong.

It's not a policy engine. It's a policy *explainer*.

### The Problem

Developers see cryptic errors:

```
Error: connection timed out
Error: admission webhook denied the request
Error: exceeded quota
```

Finding the cause means searching across NetworkPolicies, Gatekeeper constraints, Kyverno policies, Istio AuthorizationPolicies, ResourceQuotas, LimitRanges, and countless other CRDs. Even experienced platform engineers struggle to trace these issues.

### The Solution

Nightjar watches all policy-related resources in your cluster, correlates them to workloads, and provides clear explanations:

```bash
$ nightjar explain -n my-namespace "connection timed out"

Explanation: This error appears to be network-related.
Confidence: high

Matching Constraints:
  - restrict-egress (NetworkPolicy) - Severity: Critical
    Effect: Restricts egress to ports 443, 8443 only

Remediation:
  1. Request exception from platform-team@company.com
  2. Or add your workload to the allow list
```

---

## Key Features

### Universal Constraint Discovery

Nightjar discovers constraints from:

- **Kubernetes native**: NetworkPolicy, ResourceQuota, LimitRange
- **Cilium**: CiliumNetworkPolicy, CiliumClusterwideNetworkPolicy
- **Gatekeeper**: Constraints, ConstraintTemplates
- **Kyverno**: ClusterPolicy, Policy
- **Istio**: AuthorizationPolicy, PeerAuthentication
- **Webhooks**: ValidatingWebhookConfiguration, MutatingWebhookConfiguration
- **Custom CRDs**: Register any policy CRD via ConstraintProfile

### Developer-Friendly Notifications

When constraints block workloads, developers receive actionable notifications:

- **Kubernetes Events** on affected workloads
- **ConstraintReport CRDs** per namespace
- **Slack/webhook** integration for external alerting
- **Workload annotations** with constraint summaries

### Privacy-Aware Design

Multi-tenant clusters need information boundaries. Nightjar's [privacy model](/nightjar/reference/privacy/) ensures:

- Developers see what they need to unblock themselves
- Cross-namespace policy details stay hidden
- Platform admins see the full picture

### AI Agent Integration

Nightjar exposes an MCP (Model Context Protocol) server for AI coding assistants:

```
Claude: "My deployment is failing with 'connection refused'. Can you help?"

→ nightjar_explain: Matches to NetworkPolicy blocking egress on port 9090
→ nightjar_remediation: Provides kubectl command to request exception
```

### Pre-Flight Checks

Validate manifests before deploying:

```bash
$ nightjar check -f deployment.yaml

Would Block: true
Blocking Constraints:
  - require-resource-limits (Admission) - Severity: Critical
    Effect: Rejects pods without resource limits

Warnings:
  - restrict-egress: Egress will be limited to ports 443, 8443
```

---

## Quick Install

```bash
helm repo add nightjar https://nightjarctl.github.io/charts
helm install nightjar nightjar/nightjar -n nightjar-system --create-namespace
```

Then install the CLI:

```bash
go install github.com/nightjarctl/nightjar/cmd/nightjarctl@latest
```

See the [Installation Guide](/nightjar/getting-started/installation/) for detailed instructions.

---

## How It Works

1. **Discovery**: Nightjar watches all policy-related CRDs in the cluster
2. **Parsing**: Adapters normalize each policy type into a common `Constraint` model
3. **Indexing**: Constraints are indexed by namespace, labels, and type
4. **Correlation**: When errors occur, Nightjar matches them to relevant constraints
5. **Notification**: Developers receive privacy-scoped explanations with remediation steps

```
┌─────────────┐    ┌──────────────┐    ┌─────────────┐
│ NetworkPolicy│───▶│              │    │ Kubernetes  │
│ ResourceQuota│───▶│   Nightjar   │───▶│   Events    │
│ Gatekeeper  │───▶│  Controller  │───▶│ConstraintRpt│
│ Kyverno     │───▶│              │───▶│ Slack/Hook  │
│ Istio       │───▶│              │    │ Annotations │
└─────────────┘    └──────────────┘    └─────────────┘
        ▲                  │
        │                  ▼
   ┌────┴────┐      ┌──────────────┐
   │ Hubble  │      │  MCP Server  │◀───── AI Agents
   │ (flows) │      │  CLI/kubectl │◀───── Developers
   └─────────┘      └──────────────┘
```

---

## Documentation

| Section | Description |
|---------|-------------|
| [Getting Started](/nightjar/getting-started/) | Installation, prerequisites, quickstart |
| [CLI Reference](/nightjar/cli/) | nightjar query, explain, check, remediate, status |
| [Controller](/nightjar/controller/) | Configuration, adapters, notifications |
| [MCP Server](/nightjar/mcp/) | AI agent integration |
| [CRDs](/nightjar/crds/) | ConstraintReport, ConstraintProfile, NotificationPolicy |
| [Reference](/nightjar/reference/) | Constraint types, severity levels |

---

## Community

- [GitHub Issues](https://github.com/nightjarctl/nightjar/issues) - Bug reports and feature requests
- [GitHub Discussions](https://github.com/nightjarctl/nightjar/discussions) - Questions and community chat
