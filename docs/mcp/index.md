...
Nightjar exposes an MCP (Model Context Protocol) server for AI agent integration.
{: .fs-6 .fw-300 }
...
The Model Context Protocol (MCP) is a standard for AI assistants to interact with external tools and data sources. Nightjar's MCP server enables AI coding assistants to:
...
→ [Calls nightjar_explain with the error message]
...
kubectl logs -n nightjar-system -l app=nightjar-controller | grep "MCP server"
...
| Tool | Description |
|------|-------------|
| [nightjar_query](tools/#nightjar_query) | Query constraints in a namespace |
| [nightjar_explain](tools/#nightjar_explain) | Match error to constraints |
| [nightjar_check](tools/#nightjar_check) | Pre-check a manifest |
| [nightjar_list_namespaces](tools/#nightjar_list_namespaces) | List namespaces with constraints |
| [nightjar_remediation](tools/#nightjar_remediation) | Get remediation for a constraint |
...
// Step 1: Agent calls nightjar_explain
{
...
// Step 2: Agent calls nightjar_remediation for more detail
{
