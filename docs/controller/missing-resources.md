...
For example, a Deployment that exposes a port named `metrics` conventionally needs a `ServiceMonitor` so Prometheus can scrape it. If no matching ServiceMonitor exists, Nightjar flags the gap.

Results surface in two places:

- **MCP pre-check** (`nightjar_check` tool) -- returns `missing_prerequisites` immediately, with no debounce delay.
...
The `nightjar_check` tool returns missing resources in the `missing_prerequisites` array. Results are computed on demand with no debounce delay:
...
See [MCP Tools Reference](/nightjar/mcp/tools/) for the full response schema.
...
See [ConstraintReport CRD](/nightjar/crds/constraintreport/) for the full schema.

---

## Debounce Behavior

The evaluator debounces missing-resource alerts to prevent false positives during deployments. When resources are deployed via GitOps or CI/CD, the workload and its companion resources may arrive at slightly different times. Without debouncing, Nightjar would briefly flag a missing resource that is actually in-flight.
...
1. The evaluator tracks `workloadUID:ruleName` pairs with the timestamp of first detection.
2. A constraint is only emitted after the debounce window has elapsed with the resource still absent.
...
5. Stale tracking entries (older than 2x the debounce duration) are cleaned up periodically.

**MCP pre-check is not debounced.** The `nightjar_check` tool evaluates rules directly against the current cluster state, making it suitable for pre-deploy validation where immediate feedback is desired.
...
See [Configuration Reference](/nightjar/controller/configuration/) for the full Helm values.

---

## Future: Custom Rules

Custom rule registration is not yet supported. The `RequirementRule` interface is defined in `internal/types/` and currently only the four built-in rules above are registered by the controller at startup.

A future release will allow registering custom rules to detect project-specific missing resources.
