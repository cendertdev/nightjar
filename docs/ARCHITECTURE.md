...
Nightjar is deployed as a **Deployment-based operator**, not a DaemonSet. It needs to talk to the Kubernetes API server and optionally to Hubble Relay — both are network services. Running per-node would waste resources and require distributed coordination.
...
4. Check for `nightjar.io/is-policy: "true"` annotations on CRDs (admin override)
...
```
nightjar_constraints_total{type, severity, adapter}
nightjar_notifications_total{scope, channel, severity, namespace}
nightjar_discovery_duration_seconds{}
nightjar_adapter_errors_total{adapter}
nightjar_watched_resources_total{}
nightjar_hubble_flow_drops_total{namespace}
nightjar_requirement_violations_total{rule, namespace}
nightjar_notification_rate_limited_total{namespace}
```
