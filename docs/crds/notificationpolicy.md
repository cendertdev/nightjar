...
channels:
  webhook:
    enabled: true
    url: "https://your-service.example.com/nightjar-events"
```
...
spec:
  ...
  channels:
    ...
    webhook:
      enabled: true
      url: "https://pagerduty.example.com/nightjar"
```
...
1. Policies are evaluated in alphabetical order by name
2. The first matching policy applies
3. Create a `default` policy as a fallback
...
See [Privacy Model](/nightjar/reference/privacy/) for the complete privacy classification:
