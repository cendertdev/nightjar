...
```bash
nightjar remediate -n <namespace> <constraint-name> [flags]
```
...
```bash
nightjar remediate -n my-namespace restrict-egress
```
...
Steps:
  ...
  2. [kubectl] (namespace-admin)
     ...
     Command: kubectl annotate deployment my-app nightjar.io/egress-exception=requested
...
```bash
nightjar remediate -n my-namespace require-limits
```
...
```bash
nightjar remediate -n my-namespace compute-quota
```
...
```bash
nightjar remediate -n my-namespace restrict-egress -o json
```
...
```bash
nightjar remediate -n my-namespace nonexistent-constraint
```
