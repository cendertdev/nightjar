...
critical=$(nightjar status -o json | jq '.total_critical')
...
nightjar status -o json | jq -r '
  "nightjar_constraints_total " + (.total_constraints | tostring),
  "nightjar_constraints_critical " + (.total_critical | tostring),
  "nightjar_constraints_warning " + (.total_warning | tostring),
  "nightjar_constraints_info " + (.total_info | tostring)
'
```

Output:
```
nightjar_constraints_total 17
nightjar_constraints_critical 3
nightjar_constraints_warning 8
nightjar_constraints_info 6
```
...
- [Controller Status](/nightjar/controller/) - Controller health and metrics
