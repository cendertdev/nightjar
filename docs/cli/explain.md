...
```bash
nightjar explain -n <namespace> "<error-message>" [flags]
```
...
```bash
nightjar explain -n my-namespace "connection timed out to port 9090"
```
...
```bash
nightjar explain -n my-namespace "admission webhook denied the request: pods must have resource limits"
```
...
```bash
nightjar explain -n my-namespace "exceeded quota: requested cpu 2, limit 1"
```
...
```bash
nightjar explain -n my-namespace "unknown error occurred"
```
...
```bash
nightjar explain -n my-namespace "connection refused" -o json
```
