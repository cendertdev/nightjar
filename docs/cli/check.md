...
```bash
nightjar check -f <manifest-file> [flags]
```
...
```bash
nightjar check -f deployment.yaml -o json
```
...
nightjar check -f deployment.yaml -o json > check-result.json
...
for file in $(git diff --cached --name-only | grep -E '\.ya?ml$'); do
  if nightjar check -f "$file" -o json | jq -e '.would_block' > /dev/null 2>&1; then
    result=$(nightjar check -f "$file" -o json)
