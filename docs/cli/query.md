...
```bash
nightjar query -n <namespace> [flags]
```
...
The `query` command retrieves all constraints affecting workloads in a namespace. It reads from the ConstraintReport CRD created by the Nightjar controller.
...
```bash
nightjar query -n my-namespace --type NetworkEgress
```
...
```bash
nightjar query -n my-namespace --severity Critical
```
      ...
      "remediation": {
        ...
        "steps": [
          ...
          },
          {
            ...
            "command": "kubectl annotate deployment my-app nightjar.io/egress-exception=true",
