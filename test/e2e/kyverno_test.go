...
const (
	...
	kyvernoAnnotationTimeout = 120 * time.Second
)
...
// TestKyvernoClusterPolicyDiscovery verifies that creating a Kyverno ClusterPolicy
// with an Enforce validate rule causes it to be discovered, indexed as Critical
// severity, and annotated on workloads.
func (s *E2ESuite) TestKyvernoClusterPolicyDiscovery() {
	t := s.T()
	requireKyvernoInstalled(t, s.dynamicClient)
	...
	cleanupPolicy := createKyvernoClusterPolicy(
		...
		policyName, "Enforce",
		[]interface{}{
			kyvernoValidateRule("check-labels", kyvernoMatchAnyPods(), "Must have team label"),
		},
	)
	t.Cleanup(cleanupPolicy)

	// ClusterPolicies are cluster-scoped — trigger the annotator for this namespace.
	cleanupTrigger := createAnnotatorTrigger(t, s.dynamicClient, ns, "e2e-trigger-kv-disc")
	t.Cleanup(cleanupTrigger)

	constraints := waitForConstraintMatch(t, s.dynamicClient, ns, sentinelName, kyvernoAnnotationTimeout, func(c constraintSummary) bool {
...
// TestKyvernoNamespacePolicyDiscovery verifies that a namespace-scoped Kyverno
// Policy with an Audit validate rule is discovered as Warning severity and
// scoped to the policy's namespace.
func (s *E2ESuite) TestKyvernoNamespacePolicyDiscovery() {
	t := s.T()
	requireKyvernoInstalled(t, s.dynamicClient)
	...
	cleanupTrigger := createAnnotatorTrigger(t, s.dynamicClient, ns, "e2e-trigger-kv-ns")
	t.Cleanup(cleanupTrigger)
...
// TestKyvernoMultiRulePolicy verifies that a ClusterPolicy with multiple rules
// produces a separate constraint per rule, each with a unique name in the format
// "policyName/ruleName".
func (s *E2ESuite) TestKyvernoMultiRulePolicy() {
	t := s.T()
	requireKyvernoInstalled(t, s.dynamicClient)
	...
	cleanupPolicy := createKyvernoClusterPolicy(
		...
		policyName, "Enforce",
		[]interface{}{
			kyvernoValidateRule("check-team-label", kyvernoMatchAnyPods(), "Must have team label"),
			kyvernoValidateRule("check-env-label", kyvernoMatchAnyPods(), "Must have env label"),
		},
	)
	t.Cleanup(cleanupPolicy)

	cleanupTrigger := createAnnotatorTrigger(t, s.dynamicClient, ns, "e2e-trigger-kv-multi")
	t.Cleanup(cleanupTrigger)
...
// TestKyvernoMutateGenerateSeverity verifies that mutate and generate rules
// are indexed as Info severity.
func (s *E2ESuite) TestKyvernoMutateGenerateSeverity() {
	t := s.T()
	requireKyvernoInstalled(t, s.dynamicClient)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cleanupTrigger := createAnnotatorTrigger(t, s.dynamicClient, ns, "e2e-trigger-kv-"+tt.name)
			t.Cleanup(cleanupTrigger)
...
// TestKyvernoEnforcementMapping verifies that Kyverno validationFailureAction
// maps to the correct Nightjar severity levels:
//
//	Enforce → Critical
//	Audit   → Warning
func (s *E2ESuite) TestKyvernoEnforcementMapping() {
	t := s.T()
	requireKyvernoInstalled(t, s.dynamicClient)
	...
	for _, tt := range tests {
		t.Run(tt.action, func(t *testing.T) {
			...
			cleanupTrigger := createAnnotatorTrigger(t, s.dynamicClient, ns, "e2e-trigger-kv-"+strings.ToLower(tt.action))
			t.Cleanup(cleanupTrigger)
...
// TestKyvernoMatchClauseParsing verifies that ClusterPolicies with match.any
...
func (s *E2ESuite) TestKyvernoMatchClauseParsing() {
	t := s.T()
	requireKyvernoInstalled(t, s.dynamicClient)
	...
	cleanupTrigger := createAnnotatorTrigger(t, s.dynamicClient, ns1, "e2e-trigger-kv-match")
	t.Cleanup(cleanupTrigger)

	// Verify constraint appears in ns1's workload annotations.
	constraints := waitForConstraintMatch(t, s.dynamicClient, ns1, sentinelName, kyvernoAnnotationTimeout, func(c constraintSummary) bool {
		return c.Type == "Admission" && strings.Contains(c.Name, policyName)
	})
...
// TestKyvernoPolicyDeletion verifies that deleting a Kyverno ClusterPolicy
// removes all its rule-constraints from workload annotations and the ConstraintReport.
func (s *E2ESuite) TestKyvernoPolicyDeletion() {
	t := s.T()
	requireKyvernoInstalled(t, s.dynamicClient)
	...
	cleanupTrigger := createAnnotatorTrigger(t, s.dynamicClient, ns, "e2e-trigger-kv-del")
	t.Cleanup(cleanupTrigger)
	...
	err := s.dynamicClient.Resource(clusterPolicyGVR).Delete(
