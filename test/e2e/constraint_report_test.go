...
const reportCreateTimeout = 60 * time.Second
...
const reportUpdateTimeout = 45 * time.Second
...
// TestConstraintReportCreatedOnConstraint verifies that creating a NetworkPolicy
...
func (s *E2ESuite) TestConstraintReportCreatedOnConstraint() {
	t := s.T()
	...
	t.Cleanup(func() {
		_ = s.dynamicClient.Resource(npGVR).Namespace(s.namespace).Delete(ctx, "e2e-report-deny-ingress", metav1.DeleteOptions{})
	})
	...
	report := getConstraintReport(t, s.dynamicClient, s.namespace, 5*time.Second)
...
// TestConstraintReportUpdateOnConstraintChange verifies that updating a
// constraint (ResourceQuota) causes the ConstraintReport to be re-reconciled.
func (s *E2ESuite) TestConstraintReportUpdateOnConstraintChange() {
	t := s.T()
	ctx := context.Background()
	...
	_, err := s.dynamicClient.Resource(rqGVR).Namespace(s.namespace).Create(ctx, rq, metav1.CreateOptions{})
	require.NoError(t, err, "failed to create ResourceQuota")
	t.Cleanup(func() {
		_ = s.dynamicClient.Resource(rqGVR).Namespace(s.namespace).Delete(ctx, "e2e-report-quota", metav1.DeleteOptions{})
	})
	...
	rqObj, err := s.dynamicClient.Resource(rqGVR).Namespace(s.namespace).Get(ctx, "e2e-report-quota", metav1.GetOptions{})
	require.NoError(t, err)
	err = unstructured.SetNestedField(rqObj.Object, "5", "spec", "hard", "cpu")
	require.NoError(t, err)
	_, err = s.dynamicClient.Resource(rqGVR).Namespace(s.namespace).Update(ctx, rqObj, metav1.UpdateOptions{})
	require.NoError(t, err, "failed to update ResourceQuota")

	// Wait for lastUpdated to change (report was re-reconciled).
	waitForReportCondition(t, s.dynamicClient, s.namespace, reportUpdateTimeout, func(status map[string]interface{}) bool {
...
// TestConstraintReportDeleteConstraint verifies that deleting a constraint
// removes it from the ConstraintReport.
func (s *E2ESuite) TestConstraintReportDeleteConstraint() {
	t := s.T()
	ctx := context.Background()
	...
	err = s.dynamicClient.Resource(npGVR).Namespace(s.namespace).Delete(ctx, npName, metav1.DeleteOptions{})
...
// TestConstraintReportMachineReadable verifies the machineReadable section of
// the ConstraintReport contains structured data with all expected fields.
func (s *E2ESuite) TestConstraintReportMachineReadable() {
	t := s.T()
	ctx := context.Background()
	...
	t.Cleanup(func() {
		_ = s.dynamicClient.Resource(npGVR).Namespace(s.namespace).Delete(ctx, "e2e-report-mr-test", metav1.DeleteOptions{})
	})
...
// TestConstraintReportSeverityCounts verifies that the ConstraintReport
// correctly counts constraints by severity level.
func (s *E2ESuite) TestConstraintReportSeverityCounts() {
	t := s.T()
	ctx := context.Background()
	...
	t.Cleanup(func() {
		_ = s.dynamicClient.Resource(npGVR).Namespace(s.namespace).Delete(ctx, "e2e-report-severity-np", metav1.DeleteOptions{})
	})
	...
	t.Cleanup(func() {
		_ = s.dynamicClient.Resource(rqGVR).Namespace(s.namespace).Delete(ctx, "e2e-report-severity-rq", metav1.DeleteOptions{})
	})
...
// TestConstraintReportClusterScopedConstraint verifies that a cluster-scoped
// constraint (ValidatingWebhookConfiguration) appears in the ConstraintReport
// for the test namespace.
func (s *E2ESuite) TestConstraintReportClusterScopedConstraint() {
	t := s.T()
	ctx := context.Background()

	// Use a distinct name to avoid collision with cluster_scoped_test.go.
	webhookName := fmt.Sprintf("e2e-report-webhook-%s", s.namespace)
	...
	t.Logf("Cluster-scoped constraint appeared in namespace %s report", s.namespace)
}
